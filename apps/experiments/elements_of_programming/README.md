...
requires clause (think of a comment or empty define, if not supported).
...
properties. Concepts represents genera as types represent species.
...
Homogeneous functional procedure: one whose inputs are all of the same type.
...
Model: a concept is modelled by types, or a type is a model for a concept, if the
...
Type function: a mapping from a type to an affiliated type.
