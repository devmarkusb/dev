...
struct measure_gcd {
    ...
    measure_gcd()
        : legend("gcd<Q, Q>(Q(250, 1000), Q(750, 1000)") {
    }
...
inline void __reverse(_RandomAccessIter __first, _RandomAccessIter __last, random_access_iterator_tag) {
...
inline void measure_sort_n_adaptive_compares() {
    for (int n = 64; n <= 32768; n *= 4) {
        ...
        I f_buf(0);
        int n_buf(0);
        sort_n_adaptive(begin(a), size(a), f_buf, n_buf, instrumented_less<less<int>, I>(less<int>(), &c));
        print("Required ");
        print(c);
        print(" compares for ");
        print(int(size(a)));
